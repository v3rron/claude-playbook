...
# PR: Ship and Fix Loop

Open a PR for the current branch, then loop until CI is fully green and all review comments are addressed.
...
If there are uncommitted changes (check git status), stage and commit them with an appropriate message before proceeding.
...
Determine the base ref (use `git branch -r` to find `origin/main` or `origin/master`). Run `git diff <base>...HEAD` and `git log <base>..HEAD --oneline` to understand what changed. Classify the change complexity:
...
**For bug fixes:** Trace the interaction between components that caused the bug. Read the files that call into or are called by the changed code. Identify the specific mechanism of failure.

#### 3c: Gather conversation context

Scan the conversation history for context that should flow into the PR description:

- **Triggering links** -- CI failure URLs, issue-tracker tickets, error-monitor alerts, or incident tickets that motivated the work.
...
Use `gh pr create --repo <owner/repo> --base <default-branch> --head <branch>`. Keep the title under 70 chars.
...
Include brief code references. Show the mechanism, not just the symptom.]

## Production impact
[Only if the conversation contains impact data. Omit if no data available.]
...
- For each failed check, read the failure logs to diagnose the issue.
...
This step handles both human and automated review-bot comments (e.g. CodeRabbit, Copilot, and similar). Every unresolved thread blocks mergeability.
...
1. **Identify the author.** Check the first comment author login. Bot usernames typically end in `[bot]`.
...
2. **Triage the comment.** Read the comment body and the referenced code. Classify as one of:
   ...
   - **(c) Informational or stylistic** -- The comment flags metrics, asks a question, or suggests a cosmetic change not worth addressing. No code change needed.

   For bot comments specifically: bot suggestions are frequently false positives or conflict with project conventions. Do not blindly apply every suggestion. Evaluate each on its merits.

3. **Reply to the thread.** Always reply before resolving, so there is an audit trail:
   - **(a)** Reply confirming what you changed.
   - **(b)** Reply explaining why no change was made.
   - **(c)** Reply with a brief acknowledgment.
